...
import com.lagou.edu.mvcframework.interceptor.GetParamUserNameSecurityInterceptor;
import com.lagou.edu.mvcframework.interceptor.Interceptor;
import com.lagou.edu.mvcframework.interceptor.SecurityInterceptor;
...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            String baseUrl = "";
            Set<String> whitelistControl = new HashSet<>();
            if(aClass.isAnnotationPresent(LagouRequestMapping.class)) {
                ...
                if(security != null)
                    whitelistControl.addAll(Arrays.asList(security.value()));
            }
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(security != null) {
                    whitelist.addAll(Arrays.asList(security.value()));
                }
                if(whitelistControl.size() > 0) {
                    whitelist.addAll(whitelistControl);
                }
                if (whitelist.size() > 0) {
                    handler.getInterceptorList().add(new GetParamUserNameSecurityInterceptor(new UserNameAccessRule(url, whitelist)));
                }
...
//        method.invoke() //
        ...
        if(handler != null && handler.getInterceptorList() != null && handler.getInterceptorList().size() > 0) {
            for(Interceptor securityInterceptor : handler.getInterceptorList()) {
                if(!securityInterceptor.before(req,resp)) {
                    resp.getWriter().write("401 access deny");
                    return;
                }
            }
        }
